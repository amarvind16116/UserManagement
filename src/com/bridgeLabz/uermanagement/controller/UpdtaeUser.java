...
import com.bridgeLabz.usermanagement.dao.DataBaseService;
...
public class UpdtaeUser extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		PreparedStatement psmt = DataBaseService.updateData();
		...
		try {
			...
			if(!Pattern.matches("["+request.getParameter("email")+"]+", "@") || !Pattern.matches("["+request.getParameter("email")+"]+", "."))
			{
			...
			else if(!Pattern.matches("[6789][0-9]{9}", request.getParameter("mob")) || !Pattern.matches("(0/91)?[6-9][0-9]{9}", request.getParameter("mob")))
			{
