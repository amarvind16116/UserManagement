	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (rb.getPwd().equals(rb.getRepwd())) {
				if (!Pattern.matches("[" + rb.getEmail() + "]+", "@")
						|| !Pattern.matches("[" + rb.getEmail() + "]+", ".")) {
					out.println(
				...
				if (!Pattern.matches("[6789][0-9]{9}", rb.getMob())
						|| !Pattern.matches("(0/91)?[6-9][0-9]{9}", rb.getMob())) {
					out.println("<html><h3 style='color:white'>Please Enter valid Mobile Number</h3></html>");
					rd = request.getRequestDispatcher("signup.jsp");
					rd.include(request, response);
				}

				else if (rb.getEmail() == "" || rb.getPwd() == "" || rb.getMob() == "" || rb.getFname() == ""
						|| rb.getGender() == "") {
					rd = request.getRequestDispatcher("signup.jsp");
					rd.include(request, response);
				} else if (check == true) {
					out.print(
							"<html><div align='center'><h3 style='color:white'>email & Mobile Number is already exist</h3></div></html>");
					rd = request.getRequestDispatcher("signup.jsp");
					rd.include(request, response);
				} else {
					if (check == false) {
